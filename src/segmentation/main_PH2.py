...
config = OmegaConf.create({
    'img_size': 128, # org size mean 575 x 766
    'batch_size': 6,
    'seed': 42,
    'workers': 3, #3
    'lr': 0.001,
...
class PH2(torch.utils.data.Dataset):
    def __init__(self,transform,idx_list):
        ...
        self.image_paths = np.array(sorted(glob.glob(f"{self.data_path}/*/*_Dermoscopic_Image/*.bmp")))[idx_list]
        self.label_paths = np.array(sorted(glob.glob(f"{self.data_path}/*/*_lesion/*_lesion.bmp")))[idx_list]
...
def get_data():
    ...
    image_paths = sorted(glob.glob(f"{data_path}/*/*_Dermoscopic_Image/*.bmp"))
    ...
    trainset = PH2(transform=train_transform,idx_list=train_idx)
    valset = PH2(transform=val_test_transform,idx_list=val_idx)
    testset = PH2(transform=val_test_transform,idx_list=test_idx)
